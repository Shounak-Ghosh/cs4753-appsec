...
def buy_card_view(request, prod_num=0):
    if request.method == 'GET':
        ...
        # We simply comment out this section of code to prevent the XSS vunlerability
        # director is not an essential feature for functionality
...
def gift_card_view(request, prod_num=0):
    ...
    # Hack: older partner sites only support GET, so special case this.
        ...
        # Only allow POST methods for user and amount data
        if request.method != "POST":
             return HttpResponseNotAllowed(["POST"])  # Reject GET requests with 405 error
...
def use_card_view(request):
    ...
    if request.method == 'GET':
        ...
        card_data_hash = hashlib.sha256(card_data).hexdigest()
        card_query = Card.objects.filter(data_hash=card_data_hash)
        ...
        if len(card_query) == 0:
            ...
            try:
                card = Card.objects.get(data_hash=card_data_hash) # use data_hash instead of data to get the card
                card.used = True
                card.save()
            except ObjectDoesNotExist:
